...
- [AutoCode.Model/Attribute/MapperAttribute.cs](file://src/AutoCode.Model/AutoMapperModel/MapperAttribute.cs)
- [AutoCode.Model/Attribute/AutoInterfaceAttribute.cs](file://src/AutoCode.Model/InterfaceAttribute/AutoInterfaceAttribute.cs)
- [AutoCode.Model/Attribute/MapPropertyAttribute.cs](file://src/AutoCode.Model/AutoMapperModel/MapPropertyAttribute.cs)
- [AutoCode.Model/Attribute/MapValueAttribute.cs](file://src/AutoCode.Model/AutoMapperModel/MapValueAttribute.cs)
...
- 内置属性的作用、配置选项与使用方法：[Mapper]、[AutoInterface]、[MapProperty]、[MapValue] 等
- 属性的元数据提取机制、继承与组合使用方式
- 不同场景下的属性配置示例（以路径引用代替代码片段）
- 属性验证规则与默认值处理
- 自定义属性的设计原则、最佳实践与常见陷阱

AutoCode 通过源码生成器在编译期读取类型与成员上的属性元数据，自动生成映射、接口或模板化代码，从而减少样板代码并提升一致性。

## 项目结构
AutoCode 的属性系统与源码生成器分布在多个项目中：
- 属性定义位于 AutoCode.Model 的 AutoMapperModel 与 InterfaceAttribute 子命名空间
...
## 结论
AutoCode 的属性系统通过声明式配置与编译期生成，显著减少了样板代码并提升了开发效率。合理使用内置属性与扩展自定义属性，可以在保证一致性的同时灵活应对复杂映射需求。
