...
- [tools/vscode-extension/package.json](file://tools/vscode-extension/package.json)
- [tools/vscode-extension/snippets/autocode.json](file://tools/vscode-extension/snippets/autocode.json)
- [AutoCode.Engine/Config/ConfigRecommender.cs](file://src/AutoCode.Engine/Config/ConfigRecommender.cs)
- [AutoCode.Engine/Config/AutoCodeConfig.cs](file://src/AutoCode.Engine/Config/AutoCodeConfig.cs)
</cite>

## 更新摘要
**所做更改**   
- 新增了ConfigRecommender智能配置推荐功能的详细说明
- 更新了辅助工具类的架构说明，反映新的Engine模块结构
- 完善了调试和诊断功能以支持智能配置推荐系统
- 增强了故障排查指南，包含配置推荐相关的问题解决方案
- **新增**：智能配置推荐系统的集成与使用指南
...
- 持续集成中的自动化诊断与质量门禁
- **新增**：自动接口生成能力的调试与验证方法
- **新增**：VS Code扩展集成，提供智能代码片段和开发辅助功能
- **新增**：ConfigRecommender智能配置推荐系统的调试与优化
...
- **新增**：Engine核心模块：AutoCode.Engine（包含ConfigRecommender智能配置推荐）
- **新增**：VS Code扩展：tools/vscode-extension（包含package.json和代码片段）
...
subgraph "引擎核心"
E["AutoCode.Engine<br/>ConfigRecommender.cs"]
C["Config<br/>AutoCodeConfig.cs"]
end
subgraph "映射生成器"
...
subgraph "VS Code扩展"
VSC["tools/vscode-extension<br/>package.json"]
SNIP["snippets/autocode.json<br/>代码片段"]
end
M --> MG
D --> MG
H1 --> MG
...
- **新增**：智能配置推荐器（ConfigRecommender）：根据项目上下文自动生成最优配置建议
- **新增**：配置管理器（AutoCodeConfig）：管理配置加载、验证和应用
- **新增**：自动接口生成器：能够根据类定义自动生成接口实现，减少手动编码工作。
- **新增**：VS Code扩展：提供package.json配置和autocode.json代码片段，增强开发体验和代码生成效率。
...
## 架构总览
下图展示了从源码到生成代码的关键流程，以及诊断与调试的接入点，特别突出了智能配置推荐系统的集成。

```mermaid
sequenceDiagram
participant Dev as "开发者"
participant VSCode as "VS Code<br/>扩展"
participant CSProj as "C# 项目"
participant ConfigRec as "配置推荐器<br/>ConfigRecommender"
...
Dev->>VSCode : 安装VS Code扩展
VSCode-->>Dev : 提供代码片段和智能提示
Dev->>CSProj : 添加 MapperAttribute 等标记
CSProj->>ConfigRec : 获取智能配置建议
ConfigRec-->>CSProj : 返回推荐配置
CSProj->>SG : 触发增量编译
SG->>SG : 解析语法树/符号
...
Note over Dev,Out : 智能配置推荐 + 自动接口生成
```
...
### 智能配置推荐系统（新增）
- **功能概述**：ConfigRecommender 根据项目上下文和最佳实践，自动生成最优的配置建议，简化配置过程。
- **工作原理**：
  - 分析项目结构和依赖关系
  - 识别常用的配置模式和最佳实践
  - 生成针对性的配置建议和默认值
  - 支持配置的动态调整和验证
- **主要特性**：
  - 智能上下文感知：根据项目类型自动推荐合适的配置
  - 渐进式配置：从基础配置开始，逐步优化
  - 配置验证：实时检查配置的有效性和一致性
  - 自定义规则：支持用户自定义配置推荐规则

**新增** ConfigRecommender 为 AutoCode 提供了智能化的配置管理能力
...
### VS Code扩展支持（新增）
- **功能概述**：新增VS Code扩展支持，提供package.json配置和autocode.json代码片段，显著提升开发体验。
- **主要特性**：
  - package.json：定义扩展元数据、命令和配置选项
  - autocode.json：提供AutoCode相关的代码片段，加速开发过程
  - 智能提示：在编辑过程中提供AutoCode特性的智能提示
  - 快速插入：通过代码片段快速插入常用的AutoCode模式
- **使用方法**：
  - 在VS Code中安装AutoCode扩展
  - 使用Ctrl+Space或Tab键插入代码片段
  - 享受增强的AutoCode开发体验

**新增** VS Code扩展为AutoCode开发提供了更好的集成体验
...
- **新增**：智能配置推荐器依赖项目分析和配置管理模块
- **新增**：VS Code扩展依赖package.json配置和代码片段定义
...
ADemo --> Tpl
ConfigRec["配置推荐器<br/>ConfigRecommender"] --> Gen
ConfigRec --> ConfigMgr["配置管理<br/>AutoCodeConfig"]
VSC["VS Code扩展<br/>package.json"] --> Snip["代码片段<br/>autocode.json"]
Snip --> Dev["开发者工作流"]
Dev --> Gen
```
...
- **新增**：智能配置推荐优化：通过缓存推荐结果和增量更新机制，减少重复分析开销
- **新增**：VS Code扩展优化：轻量级代码片段加载，不影响编辑器性能
- **新增**：配置验证优化：异步验证和批量处理，提升配置处理效率
...
- **新增**：配置推荐失效：检查项目结构是否满足推荐条件，确认配置规则有效性
- **新增**：VS Code扩展问题：确认扩展已正确安装，检查package.json配置
...
- **症状**：代码片段不工作、智能提示缺失
- **排查步骤**：
  1. 确认VS Code扩展已正确安装和启用
  2. 检查package.json配置文件语法
  3. 验证autocode.json代码片段格式
  4. 重启VS Code以确保扩展重新加载
...
- 编写覆盖关键路径的单元测试与集成测试
- 在 CI 中集成自动化诊断与质量门禁
- **新增**：充分利用智能配置推荐功能，简化配置管理
- **新增**：充分利用自动接口生成能力，减少手动编码工作量
- **新增**：安装并使用VS Code扩展，提升开发效率和代码生成体验
...
### VS Code扩展使用指南（新增）
- **安装步骤**：
  1. 在VS Code扩展市场搜索并安装AutoCode扩展
  2. 重启VS Code以确保扩展正确加载
  3. 验证扩展功能是否正常工作
- **代码片段使用**：
  1. 在C#文件中输入AutoCode相关关键词
  2. 使用Ctrl+Space或Tab键选择代码片段
  3. 根据提示完成代码插入
- **配置选项**：
  1. 通过settings.json配置扩展行为
  2. 自定义代码片段模板
  3. 调整智能提示优先级
