# 调试与诊断

<cite>
**本文引用的文件**   
- [README.md](file://README.md)
- [AutoCode.Map/Diagnostics/DiagnosticDescriptors.cs](file://src/AutoCode.Map/Diagnostics/DiagnosticDescriptors.cs)
- [AutoCode.Map/MapperGenerator.cs](file://src/AutoCode.Map/MapperGenerator.cs)
- [AutoCode.Map/Helpers/ImmutableEquatableArray.cs](file://src/AutoCode.Map/Helpers/ImmutableEquatableArray.cs)
- [AutoCode.Map/Helpers/IncrementalValuesProviderExtensions.cs](file://src/AutoCode.Map/Helpers/IncrementalValuesProviderExtensions.cs)
- [AutoCode.MapDebug/AutoMapGenerator.cs](file://src/AutoCode.Mapdebug/AutoMapGenerator.cs)
- [AutoCode.Mapdebug/GenerateMapping.cs](file://src/AutoCode.Mapdebug/GenerateMapping.cs)
- [AutoCode.Model/AutoMapperModel/MapperAttribute.cs](file://src/AutoCode.Model/AutoMapperModel/MapperAttribute.cs)
- [AutoCode.Tests/MapGeneratorTests.cs](file://src/AutoCode.Tests/MapGeneratorTests.cs)
- [AutoCode.Tests/InterfaceGeneratorTests.cs](file://src/AutoCode.Tests/InterfaceGeneratorTests.cs)
- [AutoCode.DotTemplate.SourceGenerator/DotTemplateGenerator.cs](file://src/AutoCode.XmlTemplate.SourceGenerator/DotTemplateGenerator.cs)
- [AutoCode.DotTemplate.SourceGenerator/Extend/DiagnosticIds.cs](file://src/AutoCode.XmlTemplate.SourceGenerator/Extend/DiagnosticIds.cs)
- [APP.WebAPI/appsettings.json](file://src/APP.WebAPI/appsettings.json)
- [APP.WebAPI/appsettings.Development.json](file://src/APP.WebAPI/appsettings.Development.json)
- [AutoCode.Engine/Diagnostics/DiagnosticCollector.cs](file://src/AutoCode.Engine/Diagnostics/DiagnosticCollector.cs)
- [AutoCode.Tests.V2/CodeBuilderTests.cs](file://src/AutoCode.Tests.V2/CodeBuilderTests.cs)
- [AutoCode.Tests.V2/GeneratorTests.cs](file://src/AutoCode.Tests.V2/GeneratorTests.cs)
</cite>

## 更新摘要
**所做更改**
- 新增 V2 架构中的 DiagnosticCollector 组件说明
- 更新测试基础设施，包含 AutoCode.Tests.V2 项目
- 增强错误追踪和报告机制的文档
- 更新架构图以反映新的诊断收集器

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本指南面向使用 AutoCode 框架的开发者，聚焦于"调试与诊断"主题。内容涵盖：
- 如何使用 Source Generator 调试工具查看生成代码、定位问题
- 如何理解并处理 DiagnosticDescriptors 的诊断信息
- **新增** V2 架构中的 DiagnosticCollector 错误追踪机制
- 增强的测试基础设施（AutoCode.Tests.V2）
- 测试策略与断言验证（单元测试与集成测试）
- 常见问题排查步骤、日志配置与性能监控技巧
- 持续集成中的自动化诊断与质量门禁

## 项目结构
AutoCode 由多个工程组成，核心包括：
- 模型与属性定义：AutoCode.Model
- 映射生成器：AutoCode.Map（含诊断与增量编译辅助）
- **新增** V2 引擎：AutoCode.Engine（包含新的 DiagnosticCollector）
- 模板生成器：AutoCode.XmlTemplate.SourceGenerator
- 调试工程：AutoCode.MapDebug（用于可视化/导出中间产物）
- **增强** 测试工程：AutoCode.Tests 和 AutoCode.Tests.V2
- 示例应用：APP、APP.WebAPI 等

```mermaid
graph TB
subgraph "模型与属性"
M["AutoCode.Model<br/>MapperAttribute 等"]
end
subgraph "V2 引擎"
VE["AutoCode.Engine<br/>DiagnosticCollector.cs"]
end
subgraph "映射生成器"
MG["AutoCode.Map<br/>MapperGenerator.cs"]
D["Diagnostics<br/>DiagnosticDescriptors.cs"]
H1["Helpers<br/>ImmutableEquatableArray.cs"]
H2["Helpers<br/>IncrementalValuesProviderExtensions.cs"]
end
subgraph "模板生成器"
TG["AutoCode.XmlTemplate.SourceGenerator<br/>DotTemplateGenerator.cs"]
DI["Extend<br/>DiagnosticIds.cs"]
end
subgraph "调试工程"
DBG["AutoCode.MapDebug<br/>AutoMapGenerator.cs / GenerateMapping.cs"]
end
subgraph "测试 - V1"
T1["AutoCode.Tests<br/>MapGeneratorTests.cs"]
T2["AutoCode.Tests<br/>InterfaceGeneratorTests.cs"]
end
subgraph "测试 - V2"
TV2["AutoCode.Tests.V2<br/>CodeBuilderTests.cs / GeneratorTests.cs"]
end
subgraph "示例应用"
APP["APP.WebAPI<br/>appsettings*.json"]
end
M --> MG
D --> MG
H1 --> MG
H2 --> MG
VE --> MG
M --> TG
DI --> TG
MG --> DBG
T1 --> MG
T2 --> TG
TV2 --> VE
APP --> MG
APP --> TG
```

**图表来源**
- [AutoCode.Map/MapperGenerator.cs](file://src/AutoCode.Map/MapperGenerator.cs)
- [AutoCode.Map/Diagnostics/DiagnosticDescriptors.cs](file://src/AutoCode.Map/Diagnostics/DiagnosticDescriptors.cs)
- [AutoCode.Engine/Diagnostics/DiagnosticCollector.cs](file://src/AutoCode.Engine/Diagnostics/DiagnosticCollector.cs)
- [AutoCode.Tests.V2/CodeBuilderTests.cs](file://src/AutoCode.Tests.V2/CodeBuilderTests.cs)
- [AutoCode.Tests.V2/GeneratorTests.cs](file://src/AutoCode.Tests.V2/GeneratorTests.cs)

## 核心组件
- 映射生成器（MapperGenerator）：负责扫描标记、构建映射规则、调用模板引擎生成代码，并输出诊断信息。
- 诊断描述（DiagnosticDescriptors）：集中定义诊断 ID、严重级别、标题与消息模板，便于统一管理与本地化。
- **新增** 诊断收集器（DiagnosticCollector）：V2 架构中的核心错误追踪组件，提供统一的错误收集和报告机制。
- 增量编译辅助（Helpers）：提供不可变数组与增量值提供者扩展，提升 Source Generator 的性能与稳定性。
- 模板生成器（DotTemplateGenerator）：基于 doT 模板引擎生成 C# 代码，支持外部扩展方法。
- 调试工程（AutoCode.MapDebug）：提供中间产物导出与可视化能力，便于定位生成逻辑问题。
- **增强** 测试工程（AutoCode.Tests 和 AutoCode.Tests.V2）：覆盖映射与接口生成的关键路径，确保回归稳定。

**章节来源**
- [AutoCode.Map/MapperGenerator.cs](file://src/AutoCode.Map/MapperGenerator.cs)
- [AutoCode.Map/Diagnostics/DiagnosticDescriptors.cs](file://src/AutoCode.Map/Diagnostics/DiagnosticDescriptors.cs)
- [AutoCode.Engine/Diagnostics/DiagnosticCollector.cs](file://src/AutoCode.Engine/Diagnostics/DiagnosticCollector.cs)
- [AutoCode.Tests.V2/CodeBuilderTests.cs](file://src/AutoCode.Tests.V2/CodeBuilderTests.cs)
- [AutoCode.Tests.V2/GeneratorTests.cs](file://src/AutoCode.Tests.V2/GeneratorTests.cs)

## 架构总览
下图展示了从源码到生成代码的关键流程，以及诊断与调试的接入点，包括新的 V2 诊断收集器。

```mermaid
sequenceDiagram
participant Dev as "开发者"
participant CSProj as "C# 项目"
participant SG as "Source Generator<br/>MapperGenerator"
participant DC as "诊断收集器<br/>DiagnosticCollector (V2)"
participant Diag as "诊断描述<br/>DiagnosticDescriptors"
participant Tpl as "模板引擎<br/>DotTemplateGenerator"
participant Out as "生成代码"
participant Debug as "调试工程<br/>AutoCode.MapDebug"
Dev->>CSProj : 添加 MapperAttribute 等标记
CSProj->>SG : 触发增量编译
SG->>DC : 记录诊断信息
DC->>Diag : 格式化诊断信息
SG->>Tpl : 渲染模板生成代码
Tpl-->>SG : 返回生成文本
SG-->>Out : 写入生成代码
Dev->>Debug : 运行调试导出中间产物
Debug-->>Dev : 查看 AST/映射规则/模板结果
```

**图表来源**
- [AutoCode.Map/MapperGenerator.cs](file://src/AutoCode.Map/MapperGenerator.cs)
- [AutoCode.Engine/Diagnostics/DiagnosticCollector.cs](file://src/AutoCode.Engine/Diagnostics/DiagnosticCollector.cs)
- [AutoCode.Map/Diagnostics/DiagnosticDescriptors.cs](file://src/AutoCode.Map/Diagnostics/DiagnosticDescriptors.cs)

## 详细组件分析

### 诊断系统（DiagnosticDescriptors）
- 作用：集中管理诊断 ID、严重级别、标题与消息模板，保证一致的用户体验与可维护性。
- 使用方式：在生成器中遇到异常或边界情况时，通过统一的 API 上报诊断；IDE 会显示为错误或警告。
- 最佳实践：
  - 为每个诊断分配唯一 ID，避免冲突
  - 明确严重级别（Error/Warning/Info），便于过滤与告警
  - 消息模板包含上下文信息（如类型名、成员名、行号）

**章节来源**
- [AutoCode.Map/Diagnostics/DiagnosticDescriptors.cs](file://src/AutoCode.Map/Diagnostics/DiagnosticDescriptors.cs)

### **新增** 诊断收集器（DiagnosticCollector）
- 作用：V2 架构中的核心错误追踪组件，提供统一的错误收集和报告机制。
- 功能特性：
  - 集中式错误收集：统一管理所有诊断信息
  - 分类统计：按严重级别和类别组织错误
  - 批量报告：支持一次性输出所有诊断信息
  - 上下文保留：保持完整的错误上下文信息
- 使用场景：
  - 复杂生成过程中的错误追踪
  - 批量处理的错误汇总
  - 调试和诊断信息的结构化输出

**章节来源**
- [AutoCode.Engine/Diagnostics/DiagnosticCollector.cs](file://src/AutoCode.Engine/Diagnostics/DiagnosticCollector.cs)

### 映射生成器（MapperGenerator）
- 职责：扫描带有映射属性的类型，构建映射规则，驱动模板引擎生成代码，并输出诊断。
- 关键点：
  - 增量编译：利用 IncrementalValuesProvider 减少重复工作
  - 不可变集合：使用 ImmutableEquatableArray 降低哈希比较成本
  - 错误恢复：对无法解析的节点记录诊断，继续处理其他节点
  - **V2 集成**：与 DiagnosticCollector 集成，提供更强大的错误追踪能力

```mermaid
flowchart TD
Start(["开始"]) --> Scan["扫描标记与类型"]
Scan --> BuildRules["构建映射规则"]
BuildRules --> Validate{"规则有效？"}
Validate --> |否| Report["输出诊断错误/警告"]
Report --> Collect["收集到 DiagnosticCollector"]
Collect --> End(["结束"])
Validate --> |是| Render["渲染模板"]
Render --> Emit["生成代码"]
Emit --> End
```

**图表来源**
- [AutoCode.Map/MapperGenerator.cs](file://src/AutoCode.Map/MapperGenerator.cs)
- [AutoCode.Engine/Diagnostics/DiagnosticCollector.cs](file://src/AutoCode.Engine/Diagnostics/DiagnosticCollector.cs)

**章节来源**
- [AutoCode.Map/MapperGenerator.cs](file://src/AutoCode.Map/MapperGenerator.cs)
- [AutoCode.Map/Helpers/ImmutableEquatableArray.cs](file://src/AutoCode.Map/Helpers/ImmutableEquatableArray.cs)
- [AutoCode.Map/Helpers/IncrementalValuesProviderExtensions.cs](file://src/AutoCode.Map/Helpers/IncrementalValuesProviderExtensions.cs)

### 模板生成器（DotTemplateGenerator）
- 职责：基于 doT 模板引擎渲染 C# 代码，支持外部扩展方法注入。
- 关键点：
  - 模板变量与函数需严格校验，避免运行时错误
  - 外部扩展类应实现稳定的接口，避免破坏兼容性
  - 模板缓存与增量更新可减少重复渲染

**章节来源**
- [AutoCode.XmlTemplate.SourceGenerator/DotTemplateGenerator.cs](file://src/AutoCode.XmlTemplate.SourceGenerator/DotTemplateGenerator.cs)
- [AutoCode.XmlTemplate.SourceGenerator/Extend/DiagnosticIds.cs](file://src/AutoCode.XmlTemplate.SourceGenerator/Extend/DiagnosticIds.cs)

### 调试工程（AutoCode.MapDebug）
- 作用：导出中间产物（AST、映射规则、模板渲染结果），帮助定位生成逻辑问题。
- 使用建议：
  - 在复现问题时启用调试导出
  - 将导出的 JSON/文本作为问题附件提交
  - 结合 IDE 的"查看生成代码"功能交叉验证

**章节来源**
- [AutoCode.Mapdebug/AutoMapGenerator.cs](file://src/AutoCode.Mapdebug/AutoMapGenerator.cs)
- [AutoCode.Mapdebug/GenerateMapping.cs](file://src/AutoCode.Mapdebug/GenerateMapping.cs)

### **增强** 测试工程（AutoCode.Tests 和 AutoCode.Tests.V2）
- 覆盖范围：映射生成与接口生成的关键用例
- **V1 测试**（AutoCode.Tests）：
  - 单元测试：针对单个生成器行为进行断言
  - 集成测试：端到端验证从标记到生成代码的完整链路
  - 快照测试：对比生成代码变更，防止回归
- **V2 测试**（AutoCode.Tests.V2）：
  - 增强的测试基础设施
  - 更好的错误追踪和诊断验证
  - 改进的代码生成测试

**章节来源**
- [AutoCode.Tests/MapGeneratorTests.cs](file://src/AutoCode.Tests/MapGeneratorTests.cs)
- [AutoCode.Tests/InterfaceGeneratorTests.cs](file://src/AutoCode.Tests/InterfaceGeneratorTests.cs)
- [AutoCode.Tests.V2/CodeBuilderTests.cs](file://src/AutoCode.Tests.V2/CodeBuilderTests.cs)
- [AutoCode.Tests.V2/GeneratorTests.cs](file://src/AutoCode.Tests.V2/GeneratorTests.cs)

## 依赖关系分析
- 生成器依赖模型属性（如 MapperAttribute）以识别目标类型
- 诊断模块被生成器与模板引擎共同引用，确保一致的错误报告
- **新增** V2 诊断收集器被生成器引用，提供增强的错误追踪
- 调试工程依赖生成器的中间产物导出接口
- 测试工程依赖生成器与模板引擎的公共 API

```mermaid
graph LR
Attr["模型属性<br/>MapperAttribute"] --> Gen["映射生成器<br/>MapperGenerator"]
Gen --> Diag["诊断<br/>DiagnosticDescriptors"]
Gen --> DC["诊断收集器<br/>DiagnosticCollector (V2)"]
Gen --> Tpl["模板引擎<br/>DotTemplateGenerator"]
Tpl --> DiagExt["诊断ID扩展<br/>DiagnosticIds"]
Gen --> Debug["调试导出<br/>AutoCode.MapDebug"]
Tests["测试 V1<br/>AutoCode.Tests"] --> Gen
Tests --> Tpl
TestsV2["测试 V2<br/>AutoCode.Tests.V2"] --> DC
TestsV2 --> Gen
```

**图表来源**
- [AutoCode.Model/AutoMapperModel/MapperAttribute.cs](file://src/AutoCode.Model/AutoMapperModel/MapperAttribute.cs)
- [AutoCode.Map/MapperGenerator.cs](file://src/AutoCode.Map/MapperGenerator.cs)
- [AutoCode.Engine/Diagnostics/DiagnosticCollector.cs](file://src/AutoCode.Engine/Diagnostics/DiagnosticCollector.cs)
- [AutoCode.Tests.V2/CodeBuilderTests.cs](file://src/AutoCode.Tests.V2/CodeBuilderTests.cs)

## 性能考量
- 增量编译：充分利用 IncrementalValuesProvider 与不可变集合，减少不必要的重新计算
- 模板渲染：缓存已渲染片段，避免重复 IO 与字符串拼接
- 诊断输出：**V2 优化** 使用 DiagnosticCollector 批量处理诊断信息，减少 I/O 操作
- 内存占用：限制 AST 与中间对象的存活时间，及时释放
- **新增** 错误追踪优化：DiagnosticCollector 提供高效的错误收集和报告机制

## 故障排查指南

### 常见症状与定位
- 未生成代码：检查是否缺少必要的标记属性或命名空间
- 生成代码不正确：查看模板变量是否正确注入，是否存在类型不匹配
- 编译报错：根据诊断 ID 与消息定位具体位置，必要时启用调试导出
- **V2 新增** 使用 DiagnosticCollector 获取详细的错误追踪信息

### 诊断信息使用方法
- 在 IDE 中查看"错误列表"，按严重级别筛选
- 复制诊断 ID 与消息，结合源码上下文快速定位
- 对于复杂问题，导出中间产物并附加到问题单
- **V2 增强** 使用 DiagnosticCollector 获取结构化的错误信息

### 日志配置与性能监控
- 应用层日志：在 appsettings.json 与 appsettings.Development.json 中配置日志级别与输出目标
- 生成器日志：在调试工程中开启详细导出，观察 AST 与模板渲染过程
- 性能监控：关注编译耗时与内存峰值，优化热点路径
- **V2 新增** 诊断收集器日志：监控错误收集和报告的性能

**章节来源**
- [APP.WebAPI/appsettings.json](file://src/APP.WebAPI/appsettings.json)
- [APP.WebAPI/appsettings.Development.json](file://src/APP.WebAPI/appsettings.Development.json)
- [AutoCode.Map/Diagnostics/DiagnosticDescriptors.cs](file://src/AutoCode.Map/Diagnostics/DiagnosticDescriptors.cs)
- [AutoCode.Engine/Diagnostics/DiagnosticCollector.cs](file://src/AutoCode.Engine/Diagnostics/DiagnosticCollector.cs)

## 结论
通过合理使用诊断系统、调试工程与测试套件，可以高效定位与修复 AutoCode 生成过程中的问题。**V2 架构的引入显著增强了错误追踪和诊断能力**。建议在开发过程中：
- 始终启用增量编译与最小化诊断输出
- 使用调试工程导出中间产物以辅助定位
- **V2 推荐** 使用 DiagnosticCollector 进行集中的错误追踪
- 编写覆盖关键路径的单元测试与集成测试
- **V2 测试** 利用增强的测试基础设施进行更全面的验证
- 在 CI 中集成自动化诊断与质量门禁

## 附录

### 单元测试编写指南
- 目标：验证生成器对输入标记的处理是否符合预期
- 要点：
  - 构造最小化的输入样例
  - 断言生成代码的结构与内容
  - 覆盖边界条件与异常路径
  - **V2 新增** 使用 DiagnosticCollector 验证错误追踪

**章节来源**
- [AutoCode.Tests/MapGeneratorTests.cs](file://src/AutoCode.Tests/MapGeneratorTests.cs)
- [AutoCode.Tests/InterfaceGeneratorTests.cs](file://src/AutoCode.Tests/InterfaceGeneratorTests.cs)
- [AutoCode.Tests.V2/CodeBuilderTests.cs](file://src/AutoCode.Tests.V2/CodeBuilderTests.cs)
- [AutoCode.Tests.V2/GeneratorTests.cs](file://src/AutoCode.Tests.V2/GeneratorTests.cs)

### 集成测试策略
- 目标：端到端验证从标记到生成代码的完整链路
- 要点：
  - 使用真实的项目结构与依赖
  - 模拟编译与生成过程
  - 对比生成结果与期望快照
  - **V2 增强** 集成 DiagnosticCollector 进行错误追踪验证

**章节来源**
- [AutoCode.Tests/MapGeneratorTests.cs](file://src/AutoCode.Tests/MapGeneratorTests.cs)
- [AutoCode.Tests/InterfaceGeneratorTests.cs](file://src/AutoCode.Tests/InterfaceGeneratorTests.cs)
- [AutoCode.Tests.V2/GeneratorTests.cs](file://src/AutoCode.Tests.V2/GeneratorTests.cs)

### 持续集成配置建议
- 步骤：
  - 还原依赖并构建解决方案
  - 运行单元测试与集成测试
  - 收集诊断与日志作为构建产物
  - 失败时阻断合并请求
  - **V2 新增** 收集 DiagnosticCollector 的输出进行分析