...
## 更新摘要
**变更内容**   
- 新增AutoCode_InterfacePrefix配置选项，支持自定义接口命名前缀
- 更新InterfaceGenerator组件的命名约定处理逻辑
- 增强接口命名约定的灵活性，支持项目特定的命名规范
...
- 泛型接口与复杂方法签名的处理方式
- 接口继承、命名约定与错误处理的最佳实践
- **新增** AutoCode_InterfacePrefix 配置选项，支持自定义接口命名前缀
...
- 源生成器
  - InterfaceGenerator：编译期入口，负责语法树遍历、筛选带特性的接口、解析签名与泛型、收集约束，并驱动后续构建。**新增** 支持AutoCode_InterfacePrefix配置，允许自定义接口命名前缀。
...
Generator->>Generator : "应用AutoCode_InterfacePrefix配置"
...
- 职责
  ...
  - **新增** 处理AutoCode_InterfacePrefix配置，应用自定义接口命名前缀。
- 关键点
  - 增量更新：仅对变更的语法节点重新处理，提升性能。
  - 诊断信息：对非法用法给出清晰的编译期诊断。
  - 过滤规则：结合 AutoIgnoreAttribute 与特性配置决定最终纳入集合的成员。
  - **新增** 命名约定处理：支持通过AutoCode_InterfacePrefix配置自定义接口命名前缀。

**更新** InterfaceGenerator现已支持AutoCode_InterfacePrefix配置，允许开发者自定义生成的接口命名前缀，满足不同项目的命名规范要求。
...
- 使用方式
  ...
  - **新增** 配置AutoCode_InterfacePrefix以自定义接口命名前缀。
...
- 命名
  ...
  - **新增** 支持AutoCode_InterfacePrefix配置，允许统一设置接口命名前缀。
  - 建议统一命名空间组织，避免跨项目命名冲突。

**更新** 命名约定功能已增强，现在支持通过AutoCode_InterfacePrefix配置统一设置接口命名前缀，满足企业级项目的命名规范要求。
...
- 常见错误
  ...
  - 命名冲突、重复定义、访问修饰符不一致。
  - **新增** AutoCode_InterfacePrefix配置无效或格式错误的诊断。
- 诊断策略
  - 编译期诊断：提供明确错误消息与定位行号。
...
- **新增** 配置处理优化：AutoCode_InterfacePrefix配置在初始化时处理，避免重复计算。
...
- 常见问题
  ...
  - **新增** AutoCode_InterfacePrefix配置问题：检查配置格式是否正确，前缀是否符合命名规范。
- 调试步骤
  - 启用详细日志与诊断输出。
  - 最小化复现：剥离无关代码，聚焦问题点。
  - 单元测试：参考 InterfaceGeneratorTests.cs 的断言模式验证生成结果。
  - **新增** 配置验证：检查AutoCode_InterfacePrefix配置在项目配置文件中的正确性。
...
## 结论
接口生成器通过特性驱动的编译期代码生成，显著降低了手写样板代码的成本。合理配置 AutoInterfaceAttribute、正确使用 AutoIgnoreAttribute、遵循命名与继承约定，并结合完善的错误处理与测试，可获得稳定高效的开发体验。**新增的AutoCode_InterfacePrefix配置功能进一步增强了命名约定的灵活性，使接口生成器能够更好地适应不同项目的命名规范需求。**
