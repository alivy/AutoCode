...
## 更新摘要
**所做更改**
- 增强了 ControllerGenerator 的 HTTP 方法推断逻辑，提供更精确的方法映射
- ValidationGenerator 新增对 EmailAddress 属性的支持，完善数据验证功能
- **新增** NativeAOT 和裁剪兼容性支持，通过 IsAotCompatible=true 消除 IL2026 警告
- **新增** 纯 if/else 语句生成替代基于反射的验证，提升 AOT 兼容性
- 更新了相关组件的职责说明和协作流程
...
- **新增** ControllerGenerator 的增强 HTTP 方法推断逻辑
- **新增** ValidationGenerator 的 EmailAddress 属性支持
- **新增** NativeAOT 和裁剪兼容性支持，消除 IL2026 警告
...
- **新增** Web API 生成器：位于 AutoCode.WebApi，包含增强的 ControllerGenerator
- **新增** 验证生成器：位于 AutoCode.Validation，包含支持 EmailAddress 的 ValidationGenerator
...
C1["ControllerGenerator<br/>增强的HTTP方法推断"]
end
subgraph "验证生成器"
V1["ValidationGenerator<br/>EmailAddress支持<br/>NativeAOT兼容"]
end
subgraph "应用示例"
A1["APP.Program"]
...
- **新增** ControllerGenerator：增强的控制器生成器，具备精确的 HTTP 方法推断逻辑，能够根据接口方法签名自动生成对应的 RESTful 端点。
- **新增** ValidationGenerator：验证生成器，现在支持 EmailAddress 属性，能够自动生成相应的数据验证逻辑，并具备 NativeAOT 兼容性。

职责边界
- Generator 关注"何时、在哪里、如何触发生成"
- Builder 关注"如何将语法树转为结构化规范"
- Spec 关注"单个接口的完整元数据与约束"
- **新增** ControllerGenerator 关注"HTTP 路由与方法映射"
- **新增** ValidationGenerator 关注"数据验证规则生成与 NativeAOT 兼容性"
...
## 架构总览
下图展示了从源码到生成代码的关键流程：编译器发现带有 [AutoInterface] 的接口，生成器解析其语法树，构建接口规范，再由构建器产出目标代码。**新增** 的 ControllerGenerator 和 ValidationGenerator 分别处理 HTTP 路由和数据验证的自动生成，同时确保 NativeAOT 兼容性。
...
participant Ctrl as "ControllerGenerator"
participant Val as "ValidationGenerator"
...
Gen->>Val : "触发验证生成"
Ctrl->>Output : "生成控制器代码"
Val->>Output : "生成验证代码<br/>NativeAOT兼容"
Gen->>Output : "写入生成的代码文件"
...
- **新增功能**：Email 地址属性支持与 NativeAOT 兼容性
- 职责：
  - 根据数据注解自动生成验证逻辑
  - 支持多种验证属性（Required、StringLength、Range 等）
  - **新增** 支持 EmailAddress 属性验证
  - 生成验证规则和错误消息
  - **新增** 通过 IsAotCompatible=true 确保 NativeAOT 兼容性
  - **新增** 生成纯 if/else 语句替代反射-based 验证，消除 IL2026 警告
- 关键特性：
  - 全面的验证属性支持
  - 自定义验证规则扩展
  - 异步验证支持
...
InterfaceGenerator --> ControllerGenerator : "触发"
InterfaceGenerator --> ValidationGenerator : "触发"
```
...
Gen->>Ctrl : "生成控制器代码"
Gen->>Val : "生成验证代码"
Ctrl->>Ctrl : "推断HTTP方法和路由"
Val->>Val : "处理EmailAddress等属性"
Val->>Val : "确保NativeAOT兼容性"
Ctrl-->>Gen : "返回控制器生成结果"
Val-->>Gen : "返回验证生成结果"
Gen->>Gen : "整合所有生成结果"
Gen-->>App : "完成参与后续编译"
...
- 与依赖注入、控制器、服务层无缝集成
- **新增** 增强的 HTTP 方法推断提供更精确的 RESTful 端点生成
- **新增** EmailAddress 验证支持完善数据验证体系
- **新增** NativeAOT 和裁剪兼容性确保现代部署需求
...
G --> C1["契约实现类"]
G --> C2["辅助工具类"]
G --> C3["控制器类<br/>增强的HTTP推断"]
G --> V1["验证类<br/>EmailAddress支持<br/>NativeAOT兼容"]
C1 --> DI["依赖注入容器"]
C2 --> APP["应用逻辑"]
C3 --> API["RESTful API"]
V1 --> VALID["数据验证"]
...
- **新增** 验证生成器依赖：
  ...
  - **新增** ValidationGenerator 具备 NativeAOT 兼容性支持
- 应用层依赖：
...
Gen --> Ctrl["ControllerGenerator<br/>增强的HTTP推断"]
Gen --> Val["ValidationGenerator<br/>EmailAddress支持<br/>NativeAOT兼容"]
Spc --> Out["生成代码"]
Out --> App["APP / WebAPI"]
```
...
- 常见问题
  ...
  - **新增** HTTP 方法推断错误：检查接口方法签名是否符合 RESTful 规范
  - **新增** EmailAddress 验证失败：确认使用了正确的验证属性和格式
  - **新增** NativeAOT 兼容性错误：检查是否启用了裁剪和 AOT 编译
  - **新增** IL2026 警告：确认生成器已切换到纯 if/else 语句模式
- 调试技巧
  - 使用单元测试验证生成结果（参考 InterfaceGeneratorTests）
  - 开启生成器诊断输出，定位解析失败点
  - 逐步缩小问题范围，隔离最小复现用例
  - **新增** 检查控制器生成的路由映射是否正确
  - **新增** 验证 EmailAddress 属性的输入格式
  - **新增** 验证 NativeAOT 兼容性设置
- 常见错误与修复
  - 泛型约束不合法：修正约束语法或移除非法约束
  - 访问修饰符不一致：统一成员的可见性策略
  - 命名空间冲突：调整命名空间或前缀/后缀策略
  - **新增** HTTP 路由冲突：检查路由模板的唯一性
  - **新增** 验证规则冲突：合并或重写冲突的验证逻辑
  - **新增** AOT 兼容性错误：确保不使用反射和动态加载
...
## 结论
AutoCode 的接口自动生成通过声明式属性与清晰的生成器架构，实现了高内聚、低耦合的代码生成体系。InterfaceGenerator、InterfaceBuilder、InterfaceSpec 三者分工明确，配合良好的增量编译与缓存策略，能够在大型项目中保持高效与稳定。**新增** 的 ControllerGenerator 和 ValidationGenerator 进一步增强了系统的功能完整性，提供了更精确的 HTTP 方法推断和完善的 EmailAddress 验证支持。**新增** 的 NativeAOT 和裁剪兼容性支持确保了现代部署需求，通过 IsAotCompatible=true 和纯 if/else 语句生成消除了 IL2026 警告。合理使用 [AutoInterface] 与 [AutoIgnore]，并结合依赖注入与分层架构，可以显著提升开发效率与代码质量。
...
- 使用示例
  ...
  - **新增** 利用增强的 ControllerGenerator 自动生成 RESTful 端点
  - **新增** 使用 ValidationGenerator 的 EmailAddress 属性进行数据验证
  - **新增** 配置 NativeAOT 兼容性以支持现代部署场景
- 参考文件
  - [Program.cs](file://src/APP/Program.cs)
  - [AutoInterface.cs](file://src/APP/AutoInterface.cs)
  - [AutoInterClass.cs](file://src/APP/AutoInterClass.cs)
  ...
  - **新增** [ValidationGenerator.cs](file://src/AutoCode.Validation/ValidationGenerator.cs)