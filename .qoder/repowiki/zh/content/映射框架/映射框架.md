...
## 更新摘要
**所做更改**   
- 新增 AutoCode_MapMethodName 配置功能，允许自定义映射方法命名约定
- 更新了 MapperGenerator 组件分析，包含新的配置选项说明
- 增强了配置与策略章节，添加了方法命名约定的详细说明
...
**更新** 新增了 AutoCode_MapMethodName 配置功能，允许开发者自定义生成的映射方法命名约定，提供更灵活的代码生成控制。

## 项目结构
AutoCode 的映射能力由"模型定义 + 源码生成器"两部分组成：
- 模型定义位于 AutoCode.Model/AutoMapperModel，包含各类特性与策略枚举，用于声明映射行为。
- 源码生成器位于 AutoCode.Map，核心为 MapperGenerator，负责解析特性并生成映射实现。
...
- 源码生成器（实现层）
  - MapperGenerator：解析带有 [Mapper]/[MapProperty] 的类型与成员，结合策略生成高性能映射代码（避免反射开销），并输出诊断信息。**新增** 支持 AutoCode_MapMethodName 配置，允许自定义生成的映射方法命名约定。
...
**更新** 源码生成器现在支持通过 AutoCode_MapMethodName 配置来自定义映射方法的命名约定，提供了更灵活的代码生成控制。
...
## 架构总览
AutoCode 映射框架采用"声明式特性 + 编译期源码生成"的架构：
- 开发者在模型上标注 [Mapper]、[MapProperty] 等特性，描述映射意图。
- 源码生成器在编译时读取这些特性与类型信息，生成高效的映射实现代码。
- **新增** 支持通过 AutoCode_MapMethodName 配置自定义映射方法命名约定。
...
participant Config as "AutoCode_MapMethodName配置"
participant Diag as "DiagnosticDescriptors"
participant App as "应用代码"
Dev->>Model : 标注 [Mapper]/[MapProperty]
Gen->>Model : 解析特性与类型信息
Gen->>Config : 读取方法命名约定配置
Gen->>Diag : 记录诊断信息(警告/错误)
Gen-->>App : 生成映射方法(使用自定义命名)
App->>Gen : 调用生成的映射方法
Gen-->>App : 返回映射结果
```
...
**更新** 新增了 AutoCode_MapMethodName 配置选项，允许开发者自定义生成的映射方法命名约定，提供更好的代码生成灵活性。
...
- MapperGenerator
  ...
  - 集成：与诊断系统协作，输出清晰的错误与警告信息。
  - **新增** 支持 AutoCode_MapMethodName 配置，允许自定义映射方法命名约定。

- DiagnosticDescriptors
  - 职责：集中管理诊断 ID、消息模板与严重级别，统一错误提示格式。
  - 使用：在解析失败、类型不匹配、策略冲突等场景输出诊断。

**更新** MapperGenerator 现在集成了 AutoCode_MapMethodName 配置功能，可以根据配置自定义生成的映射方法名称。
...
**更新** 在映射方法生成过程中，现在会检查 AutoCode_MapMethodName 配置来决定最终的方法命名。
...
- 生成器层（AutoCode.Map）
  ...
  - **新增** 集成 AutoCode_MapMethodName 配置解析。
...
```mermaid
graph LR
Model["模型层(AutoCode.Model/AutoMapperModel)"] --> Generator["生成器(AutoCode.Map)"]
Generator --> Example["示例(APP.Map, Auto.MapModels)"]
Generator --> Debug["调试(AutoCode.MapDebug)"]
Example --> Runtime["运行时调用"]
Debug --> Developer["开发者调试"]
Config["AutoCode_MapMethodName配置"] --> Generator
```

**更新** 新增了 AutoCode_MapMethodName 配置对生成器的依赖关系。
...
- **新增** 自定义方法命名：通过 AutoCode_MapMethodName 配置优化生成的代码可读性和维护性。
...
- 常见错误
  ...
  - **新增** 方法命名问题：检查 AutoCode_MapMethodName 配置是否正确。
...
- 调试技巧
  ...
  - 简化映射规则，逐步定位问题。
  - 使用单元测试验证映射结果。
  - **新增** 验证 AutoCode_MapMethodName 配置是否符合预期。
...
## 结论
AutoCode 的自动映射框架通过声明式特性与源码生成技术，提供了高效、灵活的对象映射解决方案。其支持复杂类型映射、枚举转换、属性重命名与条件映射等丰富场景，并通过诊断与调试工具提升开发体验。**新增的 AutoCode_MapMethodName 配置功能**进一步增强了代码生成的灵活性，允许开发者自定义映射方法的命名约定。建议在实际项目中合理配置策略，优化映射性能，并利用调试工具快速定位问题。
...
- 最佳实践
  ...
  - **新增** 方法命名规范：合理使用 AutoCode_MapMethodName 配置，确保生成的方法名称符合项目规范。