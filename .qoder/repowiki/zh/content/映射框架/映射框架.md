...
- 源码生成器（实现层）
  - MapperGenerator：解析带有 [Mapper]/[MapProperty] 的类型与成员，结合策略生成高性能映射代码（避免反射开销），并输出诊断信息。
...
## 结论
AutoCode 的自动映射框架通过声明式特性与源码生成技术，提供了高效、灵活的对象映射解决方案。其支持复杂类型映射、枚举转换、属性重命名与条件映射等丰富场景，并通过诊断与调试工具提升开发体验。建议在实际项目中合理配置策略，优化映射性能，并利用调试工具快速定位问题。
