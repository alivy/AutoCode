# 扩展开发

<cite>
**本文引用的文件**
- [README.md](file://README.md)
- [AutoCode.Model.csproj](file://src/AutoCode.Model/AutoCode.Model.csproj)
- [ICodeGenRecipe.cs](file://src/AutoCode.Analyzers/Recipes/ICodeGenRecipe.cs)
- [CustomRecipeAdapter.cs](file://src/AutoCode.Analyzers/Recipes/CustomRecipeAdapter.cs)
- [BuiltInRecipes.cs](file://src/AutoCode.Analyzers/Recipes/BuiltInRecipes.cs)
- [CodeGenRecipe.cs](file://src/AutoCode.Model/CodeGenRecipe.cs)
- [CustomGenerateAttribute.cs](file://src/AutoCode.Model/CustomGenerateAttribute.cs)
- [RecipeConfigLoader.cs](file://src/AutoCode.Model/RecipeConfigLoader.cs)
- [autocode.json](file://autocode.json)
- [CustomRecipeGenerator.cs](file://src/AutoCode.Generators/CustomRecipeGenerator.cs)
- [SimpleTemplateEngine.cs](file://src/AutoCode.Engine/SimpleTemplateEngine.cs)
- [AuditService.liquid](file://templates/AuditService.liquid)
- [Repository.liquid](file://templates/Repository.liquid)
- [MapperGenerator.cs](file://src/AutoCode.Map/MapperGenerator.cs)
- [DiagnosticDescriptors.cs](file://src/AutoCode.Map/Diagnostics/DiagnosticDescriptors.cs)
- [ImmutableEquatableArray.cs](file://src/AutoCode.Map/Helpers/ImmutableEquatableArray.cs)
- [IncrementalValuesProviderExtensions.cs](file://src/AutoCode.Map/Helpers/IncrementalValuesProviderExtensions.cs)
- [AutoCode.DotTemplate.SourceGenerator.csproj](file://src/AutoCode.XmlTemplate.SourceGenerator/AutoCode.DotTemplate.SourceGenerator.csproj)
- [DotTemplateGenerator.cs](file://src/AutoCode.XmlTemplate.SourceGenerator/DotTemplateGenerator.cs)
- [CSData.cs](file://src/AutoCode.XmlTemplate.SourceGenerator/CSData.cs)
- [SyntaxNodeConvert.cs](file://src/AutoCode.XmlTemplate.SourceGenerator/SyntaxNodeConvert.cs)
- [doT.js](file://src/AutoCode.XmlTemplate.External/doT.js)
- [JsonHelp.cs](file://src/AutoCode.XmlTemplate.External/JsonHelp.cs)
- [DotHelp.cs](file://src/AutoCode.XmlTemplate.External/DotHelp.cs)
- [AutoTemplate.cs](file://src/DotTemplate.APP/AutoTemplate.cs)
- [DotTemplate.cs](file://src/DotTemplate.APP/DotTemplate.cs)
- [IAutoTemplate.cs](file://src/DotTemplate.APP/IAutoTemplate.cs)
- [Program.cs](file://src/DotTemplate.APP/Program.cs)
- [DependencyInjectionServiceCollectionExtensions.cs](file://src/APP.WebAPI.Core/DependencyInjection/DependencyInjectionServiceCollectionExtensions.cs)
- [IDependencyBase.cs](file://src/APP.WebAPI.Core/DependencyInjection/LifeType/IDependencyBase.cs)
- [IScoped.cs](file://src/APP.WebAPI.Core/DependencyInjection/LifeType/IScoped.cs)
- [ISingleton.cs](file://src/APP.WebAPI.Core/DependencyInjection/LifeType/ISingleton.cs)
- [ITransient.cs](file://src/APP.WebAPI.Core/DependencyInjection/LifeType/ITransient.cs)
- [ServiceCollectionExtensions.cs](file://src/APP.WebAPI.Core/Application/Extensions/ServiceCollectionExtensions.cs)
- [AppCore.cs](file://src/APP.WebAPI.Core/Application/AppCore.cs)
</cite>

## 更新摘要
**变更内容**
- 新增自定义代码生成配方系统，包括 ICodeGenRecipe 接口、CustomGenerateAttribute 属性、RecipeConfigLoader 配置加载器
- 扩展了插件架构，支持通过 autocode.json 声明式定义自定义生成规则
- 增强了模板引擎集成，支持 Liquid 模板语法和动态模板渲染
- 完善了 IDE 集成，提供 Ctrl+. 智能推荐功能

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件面向希望在 AutoCode 框架中开发自定义扩展的开发者，系统讲解如何创建自定义属性、扩展 Source Generator、为模板引擎添加自定义函数、以及扩展依赖注入容器。文档同时阐述插件架构、事件系统与扩展机制，并提供从示例到打包发布与版本管理的完整实践路径。读者无需深入源码即可上手扩展开发，同时提供源码级参考以便进阶定制。

**更新** 新增了基于配方的自定义代码生成系统，允许通过配置文件声明式定义代码生成规则，无需编写复杂的生成器代码。

## 项目结构
AutoCode 采用"模型定义 + 源生成器 + 模板引擎 + 应用集成"的分层组织方式：
- 模型与属性定义位于 AutoCode.Model，用于声明映射策略、接口生成标记、模板绑定等元数据。
- 源生成器实现位于 AutoCode.Map 与 AutoCode.XmlTemplate.SourceGenerator，分别负责 Map 代码生成与 Dot 模板渲染。
- 模板引擎外部库与帮助方法位于 AutoCode.XmlTemplate.External，封装 doT.js 与 JSON 处理。
- 应用层示例位于 DotTemplate.APP 与 APP.WebAPI.Core，演示模板调用与依赖注入扩展点。

```mermaid
graph TB
subgraph "模型与属性"
M1["AutoCode.Model<br/>映射/接口/模板属性"]
end
subgraph "源生成器"
G1["AutoCode.Map<br/>MapperGenerator"]
G2["AutoCode.XmlTemplate.SourceGenerator<br/>DotTemplateGenerator"]
G3["CustomRecipeGenerator<br/>自定义配方生成器"]
end
subgraph "模板引擎"
T1["AutoCode.XmlTemplate.External<br/>doT.js/JSON工具"]
T2["SimpleTemplateEngine<br/>Liquid模板引擎"]
end
subgraph "应用示例"
A1["DotTemplate.APP<br/>模板调用示例"]
A2["APP.WebAPI.Core<br/>DI 扩展"]
end
M1 --> G1
M1 --> G2
M1 --> G3
G2 --> T1
G3 --> T2
A1 --> G2
A2 --> A1
```

**图表来源**
- [AutoCode.Model.csproj](file://src/AutoCode.Model/AutoCode.Model.csproj)
- [MapperGenerator.cs](file://src/AutoCode.Map/MapperGenerator.cs)
- [AutoCode.DotTemplate.SourceGenerator.csproj](file://src/AutoCode.XmlTemplate.SourceGenerator/AutoCode.DotTemplate.SourceGenerator.csproj)
- [doT.js](file://src/AutoCode.XmlTemplate.External/doT.js)
- [AutoTemplate.cs](file://src/DotTemplate.APP/AutoTemplate.cs)
- [DependencyInjectionServiceCollectionExtensions.cs](file://src/APP.WebAPI.Core/DependencyInjection/DependencyInjectionServiceCollectionExtensions.cs)

章节来源
- [README.md](file://README.md)

## 核心组件
- 自定义属性（模型层）
  - 映射相关：MapperAttribute、MapPropertyAttribute、MapValueAttribute、FormatProviderAttribute、枚举/命名/必需/可见性等策略类型。
  - 模板绑定：DotTemplateAttribute。
  - 接口生成：AutoInterfaceAttribute、AutoIgnoreAttribute。
  - **新增** 自定义生成：CustomGenerateAttribute 配合 autocode.json 使用。
- 源生成器（编译期代码生成）
  - MapperGenerator：解析映射属性并生成映射代码。
  - DotTemplateGenerator：解析模板与数据模型，调用模板引擎输出目标代码。
  - **新增** CustomRecipeGenerator：处理自定义配方配置的代码生成。
- 模板引擎（运行时/生成期渲染）
  - doT.js：模板渲染核心。
  - SimpleTemplateEngine：轻量级 Liquid 模板引擎。
  - JsonHelp/DotHelp：JSON 序列化与模板辅助函数。
- 依赖注入（DI 容器扩展）
  - IDependencyBase/IScoped/ISingleton/ITransient：生命周期约定。
  - DependencyInjectionServiceCollectionExtensions：扫描并注册服务。
  - ServiceCollectionExtensions/AppCore：应用启动时的装配入口。
- **新增** 配方系统
  - ICodeGenRecipe：统一的代码生成配方接口。
  - CodeGenRecipe：声明式配置代码生成规则。
  - RecipeConfigLoader：从 autocode.json 加载配方配置。
  - BuiltInRecipes：内置 11 个生成器的配方实现。

章节来源
- [MapperAttribute.cs](file://src/AutoCode.Model/AutoMapperModel/MapperAttribute.cs)
- [MapPropertyAttribute.cs](file://src/AutoCode.Model/AutoMapperModel/MapPropertyAttribute.cs)
- [MapValueAttribute.cs](file://src/AutoCode.Model/AutoMapperModel/MapValueAttribute.cs)
- [FormatProviderAttribute.cs](file://src/AutoCode.Model/AutoMapperModel/FormatProviderAttribute.cs)
- [EnumMappingStrategy.cs](file://src/AutoCode.Model/AutoMapperModel/EnumMappingStrategy.cs)
- [MappingConversionType.cs](file://src/AutoCode.Model/AutoMapperModel/MappingConversionType.cs)
- [PropertyNameMappingStrategy.cs](file://src/AutoCode.Model/AutoMapperModel/PropertyNameMappingStrategy.cs)
- [RequiredMappingStrategy.cs](file://src/AutoCode.Model/AutoMapperModel/RequiredMappingStrategy.cs)
- [IgnoreObsoleteMembersStrategy.cs](file://src/AutoCode.Model/AutoMapperModel/IgnoreObsoleteMembersStrategy.cs)
- [MemberVisibility.cs](file://src/AutoCode.Model/AutoMapperModel/MemberVisibility.cs)
- [DotTemplateAttribute.cs](file://src/AutoCode.Model/DotFileAttribute/DotTemplateAttribute.cs)
- [AutoInterfaceAttribute.cs](file://src/AutoCode.Model/InterfaceAttribute/AutoInterfaceAttribute.cs)
- [AutoIgnoreAttribute.cs](file://src/AutoCode.Model/InterfaceAttribute/AutoIgnoreAttribute.cs)
- [CustomGenerateAttribute.cs](file://src/AutoCode.Model/CustomGenerateAttribute.cs)
- [CodeGenRecipe.cs](file://src/AutoCode.Model/CodeGenRecipe.cs)
- [RecipeConfigLoader.cs](file://src/AutoCode.Model/RecipeConfigLoader.cs)
- [ICodeGenRecipe.cs](file://src/AutoCode.Analyzers/Recipes/ICodeGenRecipe.cs)
- [BuiltInRecipes.cs](file://src/AutoCode.Analyzers/Recipes/BuiltInRecipes.cs)
- [CustomRecipeAdapter.cs](file://src/AutoCode.Analyzers/Recipes/CustomRecipeAdapter.cs)
- [MapperGenerator.cs](file://src/AutoCode.Map/MapperGenerator.cs)
- [AutoCode.DotTemplate.SourceGenerator.csproj](file://src/AutoCode.XmlTemplate.SourceGenerator/AutoCode.DotTemplate.SourceGenerator.csproj)
- [DotTemplateGenerator.cs](file://src/AutoCode.XmlTemplate.SourceGenerator/DotTemplateGenerator.cs)
- [doT.js](file://src/AutoCode.XmlTemplate.External/doT.js)
- [JsonHelp.cs](file://src/AutoCode.XmlTemplate.External/JsonHelp.cs)
- [DotHelp.cs](file://src/AutoCode.XmlTemplate.External/DotHelp.cs)
- [IDependencyBase.cs](file://src/APP.WebAPI.Core/DependencyInjection/LifeType/IDependencyBase.cs)
- [IScoped.cs](file://src/APP.WebAPI.Core/DependencyInjection/LifeType/IScoped.cs)
- [ISingleton.cs](file://src/APP.WebAPI.Core/DependencyInjection/LifeType/ISingleton.cs)
- [ITransient.cs](file://src/APP.WebAPI.Core/DependencyInjection/LifeType/ITransient.cs)
- [DependencyInjectionServiceCollectionExtensions.cs](file://src/APP.WebAPI.Core/DependencyInjection/DependencyInjectionServiceCollectionExtensions.cs)
- [ServiceCollectionExtensions.cs](file://src/APP.WebAPI.Core/Application/Extensions/ServiceCollectionExtensions.cs)
- [AppCore.cs](file://src/APP.WebAPI.Core/Application/AppCore.cs)

## 架构总览
AutoCode 的扩展体系围绕"属性驱动 + 源生成 + 模板渲染 + DI 装配"展开：
- 开发者通过自定义属性描述意图（如映射规则、模板绑定、接口生成）。
- 源生成器在编译期读取这些属性，结合语法树与增量分析，生成目标代码。
- 模板引擎在生成期或运行期根据数据模型渲染模板，产出最终代码或配置。
- DI 容器按约定自动发现并注册扩展服务，支持生命周期管理。
- **新增** 配方系统允许通过配置文件声明式定义代码生成规则，无需编程。

```mermaid
sequenceDiagram
participant Dev as "开发者"
participant Attr as "自定义属性"
participant Config as "autocode.json"
participant SG as "Source Generator"
participant TE as "模板引擎"
participant DI as "DI 容器"
participant App as "应用"
Dev->>Attr : 标注类型/成员
Dev->>Config : 定义生成配方
Attr-->>SG : 编译期元数据
Config-->>SG : 配方配置
SG->>SG : 解析属性与语法树
SG->>TE : 传入数据模型与模板
TE-->>SG : 渲染结果
SG-->>App : 生成代码
Dev->>DI : 注册扩展服务
DI-->>App : 提供实例
```

**图表来源**
- [MapperGenerator.cs](file://src/AutoCode.Map/MapperGenerator.cs)
- [DotTemplateGenerator.cs](file://src/AutoCode.XmlTemplate.SourceGenerator/DotTemplateGenerator.cs)
- [CustomRecipeGenerator.cs](file://src/AutoCode.Generators/CustomRecipeGenerator.cs)
- [RecipeConfigLoader.cs](file://src/AutoCode.Model/RecipeConfigLoader.cs)
- [doT.js](file://src/AutoCode.XmlTemplate.External/doT.js)
- [DependencyInjectionServiceCollectionExtensions.cs](file://src/APP.WebAPI.Core/DependencyInjection/DependencyInjectionServiceCollectionExtensions.cs)

## 详细组件分析

### 自定义属性扩展（模型层）
- 设计要点
  - 使用特性类承载元数据，供源生成器读取。
  - 将策略与枚举分离，便于组合与扩展。
  - 保持属性不可变与可序列化，利于跨阶段传递。
- 关键类型
  - 映射属性：MapperAttribute、MapPropertyAttribute、MapValueAttribute、FormatProviderAttribute。
  - 策略枚举：EnumMappingStrategy、MappingConversionType、PropertyNameMappingStrategy、RequiredMappingStrategy、MemberVisibility、IgnoreObsoleteMembersStrategy。
  - 模板与接口：DotTemplateAttribute、AutoInterfaceAttribute、AutoIgnoreAttribute。
  - **新增** 自定义生成：CustomGenerateAttribute 配合 autocode.json 使用。
- 扩展建议
  - 新增属性时，同步更新诊断信息与测试用例。
  - 对复杂属性提供默认值与校验逻辑。

```mermaid
classDiagram
class MapperAttribute {
+Name
+Strategy
}
class MapPropertyAttribute {
+From
+To
+Converter
}
class MapValueAttribute {
+Value
+Condition
}
class FormatProviderAttribute {
+Culture
+Format
}
class EnumMappingStrategy
class MappingConversionType
class PropertyNameMappingStrategy
class RequiredMappingStrategy
class MemberVisibility
class IgnoreObsoleteMembersStrategy
class DotTemplateAttribute {
+FileName
+Namespace
}
class AutoInterfaceAttribute
class AutoIgnoreAttribute
class CustomGenerateAttribute {
+RecipeName
}
class CodeGenRecipe {
+Name
+Title
+Icon
+Category
+Trigger
+Output
}
class RecipeTrigger {
+AttributeName
+ClassPattern
+RequiredInterfaces
+RequiredProperties
+RequiredMethods
}
class RecipeOutput {
+Template
+FileName
+Namespace
}
MapperAttribute --> EnumMappingStrategy : "使用"
MapPropertyAttribute --> MappingConversionType : "指定转换"
MapPropertyAttribute --> PropertyNameMappingStrategy : "命名策略"
MapValueAttribute --> RequiredMappingStrategy : "条件映射"
FormatProviderAttribute --> MemberVisibility : "格式化可见性"
DotTemplateAttribute --> MemberVisibility : "输出可见性"
CustomGenerateAttribute --> CodeGenRecipe : "引用"
CodeGenRecipe --> RecipeTrigger : "包含"
CodeGenRecipe --> RecipeOutput : "包含"
```

**图表来源**
- [MapperAttribute.cs](file://src/AutoCode.Model/AutoMapperModel/MapperAttribute.cs)
- [MapPropertyAttribute.cs](file://src/AutoCode.Model/AutoMapperModel/MapPropertyAttribute.cs)
- [MapValueAttribute.cs](file://src/AutoCode.Model/AutoMapperModel/MapValueAttribute.cs)
- [FormatProviderAttribute.cs](file://src/AutoCode.Model/AutoMapperModel/FormatProviderAttribute.cs)
- [EnumMappingStrategy.cs](file://src/AutoCode.Model/AutoMapperModel/EnumMappingStrategy.cs)
- [MappingConversionType.cs](file://src/AutoCode.Model/AutoMapperModel/MappingConversionType.cs)
- [PropertyNameMappingStrategy.cs](file://src/AutoCode.Model/AutoMapperModel/PropertyNameMappingStrategy.cs)
- [RequiredMappingStrategy.cs](file://src/AutoCode.Model/AutoMapperModel/RequiredMappingStrategy.cs)
- [IgnoreObsoleteMembersStrategy.cs](file://src/AutoCode.Model/AutoMapperModel/IgnoreObsoleteMembersStrategy.cs)
- [MemberVisibility.cs](file://src/AutoCode.Model/AutoMapperModel/MemberVisibility.cs)
- [DotTemplateAttribute.cs](file://src/AutoCode.Model/DotFileAttribute/DotTemplateAttribute.cs)
- [AutoInterfaceAttribute.cs](file://src/AutoCode.Model/InterfaceAttribute/AutoInterfaceAttribute.cs)
- [AutoIgnoreAttribute.cs](file://src/AutoCode.Model/InterfaceAttribute/AutoIgnoreAttribute.cs)
- [CustomGenerateAttribute.cs](file://src/AutoCode.Model/CustomGenerateAttribute.cs)
- [CodeGenRecipe.cs](file://src/AutoCode.Model/CodeGenRecipe.cs)

章节来源
- [AutoCode.Model.csproj](file://src/AutoCode.Model/AutoCode.Model.csproj)

### Source Generator 扩展点（Map 生成器）
- 扩展点
  - 在 MapperGenerator 中解析属性与语法节点，构建中间表示，再输出代码。
  - 借助 ImmutableEquatableArray 与 IncrementalValuesProviderExtensions 提升增量分析性能。
  - 通过 DiagnosticDescriptors 统一错误与警告信息。
- 扩展步骤
  - 新增属性后，在生成器中识别并转换为中间模型。
  - 编写转换逻辑与代码生成片段。
  - 补充诊断与单元测试覆盖边界情况。

```mermaid
flowchart TD
Start(["开始"]) --> Parse["解析语法树与属性"]
Parse --> BuildModel["构建中间模型"]
BuildModel --> Validate{"验证通过?"}
Validate --> |否| EmitDiag["发出诊断信息"]
Validate --> |是| Generate["生成代码"]
Generate --> End(["结束"])
EmitDiag --> End
```

**图表来源**
- [MapperGenerator.cs](file://src/AutoCode.Map/MapperGenerator.cs)
- [DiagnosticDescriptors.cs](file://src/AutoCode.Map/Diagnostics/DiagnosticDescriptors.cs)
- [ImmutableEquatableArray.cs](file://src/AutoCode.Map/Helpers/ImmutableEquatableArray.cs)
- [IncrementalValuesProviderExtensions.cs](file://src/AutoCode.Map/Helpers/IncrementalValuesProviderExtensions.cs)

章节来源
- [MapperGenerator.cs](file://src/AutoCode.Map/MapperGenerator.cs)

### 自定义配方系统（新增）
- 设计要点
  - 通过 autocode.json 声明式定义代码生成规则，无需编写 C# 代码。
  - 支持多种触发条件：属性匹配、类名模式匹配、接口要求、属性要求、方法要求。
  - 提供统一的 ICodeGenRecipe 接口，让内置和自定义配方在 IDE 中统一展示。
- 核心组件
  - CodeGenRecipe：配方配置模型，包含名称、标题、图标、分类、触发条件和输出配置。
  - RecipeTrigger：触发条件配置，支持 attributeName、classPattern、requiredInterfaces、requiredProperties、requiredMethods。
  - RecipeOutput：输出配置，支持 template、fileName、namespace 占位符。
  - CustomGenerateAttribute：在类上标记要触发的配方名称。
  - RecipeConfigLoader：从 autocode.json 解析配方配置。
  - CustomRecipeAdapter：将配置转换为 ICodeGenRecipe 接口实现。
- 工作流程
  1. 在 autocode.json 中定义 customGenerators 数组。
  2. 在类上添加 [CustomGenerate("recipeName")] 或使用属性触发。
  3. CustomRecipeGenerator 读取配置并匹配类。
  4. 使用 SimpleTemplateEngine 渲染 Liquid 模板。
  5. 生成 .g.cs 文件到指定命名空间。

```mermaid
sequenceDiagram
participant Dev as "开发者"
participant Config as "autocode.json"
participant Gen as "CustomRecipeGenerator"
participant Loader as "RecipeConfigLoader"
participant Adapter as "CustomRecipeAdapter"
participant Engine as "SimpleTemplateEngine"
participant Output as "生成文件"
Dev->>Config : 定义配方配置
Dev->>Dev : 添加 [CustomGenerate] 属性
Config-->>Gen : 读取配置文件
Gen->>Loader : 解析配方配置
Loader-->>Gen : 返回 CodeGenRecipe 列表
Gen->>Adapter : 转换为 ICodeGenRecipe
Adapter-->>Gen : 统一接口实现
Gen->>Engine : 渲染模板
Engine-->>Gen : 生成代码
Gen-->>Output : 写入 .g.cs 文件
```

**图表来源**
- [CustomRecipeGenerator.cs](file://src/AutoCode.Generators/CustomRecipeGenerator.cs)
- [RecipeConfigLoader.cs](file://src/AutoCode.Model/RecipeConfigLoader.cs)
- [CustomRecipeAdapter.cs](file://src/AutoCode.Analyzers/Recipes/CustomRecipeAdapter.cs)
- [SimpleTemplateEngine.cs](file://src/AutoCode.Engine/SimpleTemplateEngine.cs)
- [autocode.json](file://autocode.json)

章节来源
- [CustomGenerateAttribute.cs](file://src/AutoCode.Model/CustomGenerateAttribute.cs)
- [CodeGenRecipe.cs](file://src/AutoCode.Model/CodeGenRecipe.cs)
- [RecipeConfigLoader.cs](file://src/AutoCode.Model/RecipeConfigLoader.cs)
- [CustomRecipeGenerator.cs](file://src/AutoCode.Generators/CustomRecipeGenerator.cs)
- [CustomRecipeAdapter.cs](file://src/AutoCode.Analyzers/Recipes/CustomRecipeAdapter.cs)
- [autocode.json](file://autocode.json)

### 模板引擎扩展（Dot 模板与自定义函数）
- 扩展点
  - DotTemplateGenerator 负责加载模板与数据模型，调用 doT.js 进行渲染。
  - SimpleTemplateEngine 提供轻量级 Liquid 模板支持，兼容 {% for %}、{% if %}、{{ variable }} 语法。
  - 通过 Extend/DotHelp 与 External/DotHelp 暴露模板函数，支持 JSON 处理与字符串操作。
- 自定义函数
  - 在 DotHelp 中添加新函数，确保幂等与异常安全。
  - 在 CSData 中定义数据模型字段，供模板访问。
  - 在 SyntaxNodeConvert 中将语法节点转换为模板可用数据。
- 模板文件
  - 支持 Liquid 语法，可在 templates 目录下创建 .liquid 文件。
  - 使用占位符 {{ ClassName }}、{{ Namespace }}、{{ Methods }} 等。
  - 支持循环 {% for method in Methods %}...{% endfor %} 和条件判断。

```mermaid
sequenceDiagram
participant Gen as "DotTemplateGenerator"
participant Data as "CSData"
participant Help as "DotHelp/JsonHelp"
participant Engine as "doT.js/SimpleTemplateEngine"
participant Out as "输出代码"
Gen->>Data : 准备数据模型
Gen->>Help : 注册模板函数
Gen->>Engine : 渲染模板(数据, 函数)
Engine-->>Gen : 渲染结果
Gen-->>Out : 写入生成文件
```

**图表来源**
- [DotTemplateGenerator.cs](file://src/AutoCode.XmlTemplate.SourceGenerator/DotTemplateGenerator.cs)
- [CSData.cs](file://src/AutoCode.XmlTemplate.SourceGenerator/CSData.cs)
- [SyntaxNodeConvert.cs](file://src/AutoCode.XmlTemplate.SourceGenerator/SyntaxNodeConvert.cs)
- [DotHelp.cs](file://src/AutoCode.XmlTemplate.SourceGenerator/Extend/DotHelp.cs)
- [JsonHelp.cs](file://src/AutoCode.XmlTemplate.External/JsonHelp.cs)
- [doT.js](file://src/AutoCode.XmlTemplate.External/doT.js)
- [SimpleTemplateEngine.cs](file://src/AutoCode.Engine/SimpleTemplateEngine.cs)

章节来源
- [AutoCode.DotTemplate.SourceGenerator.csproj](file://src/AutoCode.XmlTemplate.SourceGenerator/AutoCode.DotTemplate.SourceGenerator.csproj)

### 依赖注入容器扩展
- 扩展点
  - 通过 IDependencyBase 及其生命周期接口（IScoped/ISingleton/ITransient）约定服务类型。
  - DependencyInjectionServiceCollectionExtensions 扫描程序集并注册服务。
  - ServiceCollectionExtensions 与 AppCore 提供应用启动时的装配入口。
- 扩展步骤
  - 实现生命周期接口以声明服务。
  - 在扩展方法中注册服务（含工厂方法与参数注入）。
  - 在应用启动时调用扩展方法进行装配。

```mermaid
classDiagram
class IDependencyBase
class IScoped
class ISingleton
class ITransient
class DependencyInjectionServiceCollectionExtensions {
+RegisterServices()
}
class ServiceCollectionExtensions {
+ConfigureServices()
}
class AppCore {
+Initialize()
}
IScoped --|> IDependencyBase
ISingleton --|> IDependencyBase
ITransient --|> IDependencyBase
DependencyInjectionServiceCollectionExtensions --> IDependencyBase : "扫描注册"
ServiceCollectionExtensions --> DependencyInjectionServiceCollectionExtensions : "调用"
AppCore --> ServiceCollectionExtensions : "启动装配"
```

**图表来源**
- [IDependencyBase.cs](file://src/APP.WebAPI.Core/DependencyInjection/LifeType/IDependencyBase.cs)
- [IScoped.cs](file://src/APP.WebAPI.Core/DependencyInjection/LifeType/IScoped.cs)
- [ISingleton.cs](file://src/APP.WebAPI.Core/DependencyInjection/LifeType/ISingleton.cs)
- [ITransient.cs](file://src/APP.WebAPI.Core/DependencyInjection/LifeType/ITransient.cs)
- [DependencyInjectionServiceCollectionExtensions.cs](file://src/APP.WebAPI.Core/DependencyInjection/DependencyInjectionServiceCollectionExtensions.cs)
- [ServiceCollectionExtensions.cs](file://src/APP.WebAPI.Core/Application/Extensions/ServiceCollectionExtensions.cs)
- [AppCore.cs](file://src/APP.WebAPI.Core/Application/AppCore.cs)

章节来源
- [DependencyInjectionServiceCollectionExtensions.cs](file://src/APP.WebAPI.Core/DependencyInjection/DependencyInjectionServiceCollectionExtensions.cs)

### 概念总览（不直接对应源码）
- 插件架构
  - 以"属性 + 生成器 + 模板 + DI"为核心，各模块松耦合，通过约定与接口协作。
  - **新增** 配方系统提供声明式扩展点，无需编程即可定义新的代码生成规则。
- 事件系统
  - 可在生成器与模板渲染的关键阶段插入钩子（例如预处理数据、后处理输出），通过委托或回调机制实现。
- 扩展机制
  - 新增属性、新增生成器、新增模板函数、新增 DI 服务均可独立演进，互不影响。
  - **新增** 通过 autocode.json 配置即可添加新的代码生成规则，降低扩展门槛。

```mermaid
flowchart TD
PStart["插件入口"] --> Attr["属性解析"]
Attr --> Config["配方配置"]
Config --> Gen["生成器执行"]
Gen --> Hook["事件钩子"]
Hook --> Template["模板渲染"]
Template --> DI["DI 装配"]
DI --> PEnd["完成"]
```

[本图为概念流程，不直接映射具体源码]

## 依赖关系分析
- 内部依赖
  - AutoCode.Model 被所有生成器与模板模块引用。
  - AutoCode.Map 与 AutoCode.XmlTemplate.SourceGenerator 依赖模型与外部模板库。
  - **新增** CustomRecipeGenerator 依赖 RecipeConfigLoader 和 SimpleTemplateEngine。
  - 应用示例依赖生成器与模板库，并通过 DI 扩展装配服务。
- 外部依赖
  - doT.js 作为模板引擎核心。
  - .NET 源生成器 API 与增量分析 API。
  - Microsoft.Extensions.DependencyInjection。

```mermaid
graph LR
Model["AutoCode.Model"] --> MapGen["AutoCode.Map"]
Model --> DotGen["AutoCode.XmlTemplate.SourceGenerator"]
Model --> CustomGen["CustomRecipeGenerator"]
CustomGen --> ConfigLoader["RecipeConfigLoader"]
CustomGen --> TemplateEngine["SimpleTemplateEngine"]
DotGen --> DoT["doT.js"]
DotGen --> Json["JsonHelp"]
App["DotTemplate.APP"] --> DotGen
WebCore["APP.WebAPI.Core"] --> App
WebCore --> DIExt["DI 扩展"]
```

**图表来源**
- [AutoCode.Model.csproj](file://src/AutoCode.Model/AutoCode.Model.csproj)
- [MapperGenerator.cs](file://src/AutoCode.Map/MapperGenerator.cs)
- [AutoCode.DotTemplate.SourceGenerator.csproj](file://src/AutoCode.XmlTemplate.SourceGenerator/AutoCode.DotTemplate.SourceGenerator.csproj)
- [doT.js](file://src/AutoCode.XmlTemplate.External/doT.js)
- [JsonHelp.cs](file://src/AutoCode.XmlTemplate.External/JsonHelp.cs)
- [AutoTemplate.cs](file://src/DotTemplate.APP/AutoTemplate.cs)
- [DependencyInjectionServiceCollectionExtensions.cs](file://src/APP.WebAPI.Core/DependencyInjection/DependencyInjectionServiceCollectionExtensions.cs)
- [CustomRecipeGenerator.cs](file://src/AutoCode.Generators/CustomRecipeGenerator.cs)
- [RecipeConfigLoader.cs](file://src/AutoCode.Model/RecipeConfigLoader.cs)
- [SimpleTemplateEngine.cs](file://src/AutoCode.Engine/SimpleTemplateEngine.cs)

章节来源
- [AutoCode.Model.csproj](file://src/AutoCode.Model/AutoCode.Model.csproj)

## 性能考量
- 增量分析
  - 使用 IncrementalValuesProviderExtensions 减少不必要的重新生成。
  - 利用 ImmutableEquatableArray 避免重复计算与内存分配。
- 模板渲染
  - 缓存已编译的模板函数，避免重复编译开销。
  - 控制数据模型大小，仅传递必要字段。
  - **新增** SimpleTemplateEngine 提供轻量级渲染，避免外部依赖。
- 源生成器
  - 尽早失败（快速校验属性与语法），减少后续处理成本。
  - 并行处理多个文件，但注意线程安全与资源竞争。
- **新增** 配方系统优化
  - 配方配置在编译时解析一次，运行时零开销。
  - 类匹配使用高效的字符串比较和模式匹配算法。
  - 模板文件通过 AdditionalFiles 提供，避免运行时文件 IO。

[本节为通用指导，不直接分析具体文件]

## 故障排查指南
- 常见问题
  - 属性未生效：检查属性是否被正确标注且生成器是否包含相应解析逻辑。
  - 模板渲染失败：确认模板语法与提供的数据模型字段一致，检查自定义函数是否正确注册。
  - DI 装配失败：确认服务实现了生命周期接口并在扩展方法中注册。
  - **新增** 配方未触发：检查 autocode.json 配置格式、类名匹配模式、属性名称是否正确。
- 诊断信息
  - 通过 DiagnosticDescriptors 定位问题，查看生成的诊断消息与位置。
  - **新增** 配方系统会在编译时输出详细的匹配日志，帮助调试配置问题。
- 调试技巧
  - 在生成器中输出中间模型与日志，逐步缩小问题范围。
  - 使用最小复现工程隔离问题。
  - **新增** 使用 `dotnet build` 查看生成文件的详细信息，确认配方是否被正确应用。

章节来源
- [DiagnosticDescriptors.cs](file://src/AutoCode.Map/Diagnostics/DiagnosticDescriptors.cs)

## 结论
AutoCode 的扩展体系以属性为中心、以源生成为核心、以模板为表达、以 DI 为装配，形成高内聚低耦合的可扩展架构。**新增的配方系统**进一步降低了扩展开发门槛，允许通过配置文件声明式定义代码生成规则，无需编写复杂的 C# 代码。遵循本文档的扩展模式与最佳实践，开发者可以高效地创建自定义属性、扩展生成器、添加模板函数与集成第三方库，并以模块化方式发布与维护扩展包。

## 附录
- 扩展开发清单
  - 新增属性：定义属性类、默认值与校验；更新生成器解析；补充诊断与测试。
  - 新增生成器：实现增量分析与代码生成；接入现有数据模型；完善错误处理。
  - 新增模板函数：在 DotHelp 中实现；确保幂等与异常安全；提供示例模板。
  - 新增 DI 服务：实现生命周期接口；在扩展方法中注册；在应用启动装配。
  - **新增** 自定义配方：在 autocode.json 中定义 customGenerators；创建 Liquid 模板；在类上添加 [CustomGenerate] 属性。
- 打包与发布
  - 使用 NuGet 打包生成器与模板扩展；维护版本号与变更日志；提供安装脚本与示例工程。
  - **新增** 配方系统可通过 NuGet 包分发，包含配置文件和模板文件。
- 版本管理
  - 语义化版本控制；向后兼容策略；弃用通知与迁移指南。
- **新增** 配方开发最佳实践
  - 使用有意义的配方名称和标题，便于 IDE 中识别。
  - 提供清晰的触发条件，避免误匹配。
  - 模板文件应包含完整的注释和使用示例。
  - 在 autocode.json 中提供详细的配置说明。

[本节为通用指导，不直接分析具体文件]